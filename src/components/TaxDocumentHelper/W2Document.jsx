...
const INTERACTIVE_FIELDS = [
  { n: 1, fieldId: "box1", value: "48,250.00" },
  { n: 2, fieldId: "box2", value: "5,790.00" },
  { n: 3, fieldId: "box3", value: "48,250.00" },
];

const STATIC_FIELDS = [
  { n: 4, label: "Social security tax withheld", value: "2,991.50" },
  { n: 5, label: "Medicare wages & tips", value: "48,250.00" },
  { n: 6, label: "Medicare tax withheld", value: "699.63" },
];

export default function W2Document({ activeN, onBoxClick }) {
  return (
    <div className="w-[600px] flex-none rounded-md border border-dm-paper-line bg-white p-[18px] text-dm-paper-ink">
      <div className="mb-[10px] flex gap-[10px]">
        <div className="w-32 flex-none rounded border border-dm-paper-line px-[10px] py-2">
          ...
          <div className="mt-[3px] text-[17px] font-bold tracking-[0.04em]">***-**-4471</div>
        </div>
        <div className="flex flex-1 items-center justify-between rounded border border-dm-paper-line px-[14px] py-2">
          <div>
            <div className="font-serif text-xl font-semibold leading-none">Form W-2</div>
            <div className="mt-[3px] text-xs text-dm-paper-muted-2">Wage and Tax Statement</div>
          </div>
          <div className="text-right">
            <div className="font-serif text-[28px] font-semibold leading-none">2025</div>
            <div className="mt-0.5 text-[10px] text-dm-paper-muted">Dept. of the Treasury — IRS</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-stretch gap-[10px]">
        <div className="flex w-[250px] flex-none flex-col gap-[10px]">
          ...
          <div className="rounded border border-dm-paper-line px-[10px] py-2">
            ...
            <div className="mt-[3px] text-[15px] font-semibold">Amina H. Warsame</div>
          </div>
        </div>

        <div className="grid flex-1 grid-cols-2 content-start gap-[10px]">
          {INTERACTIVE_FIELDS.map((field) => {
            const annotation = getAnnotation("w2", field.fieldId);
              ...
              >
                <div className="flex items-start justify-between gap-[6px]">
                  <div className="text-[11px] leading-[1.25] tracking-[0.03em] text-dm-paper-label">
                    <b className="text-[13px] text-dm-accent">{field.n}</b>&nbsp; {annotation?.label}
                  </div>
          ...
          {STATIC_FIELDS.map((field) => (
            <div key={field.n} className="rounded-lg border border-dm-paper-line pt-[9px] px-[11px] pb-[11px]">
              <div className="text-[11px] leading-[1.25] text-dm-paper-disabled">
                <b className="text-[13px] text-dm-paper-disabled">{field.n}</b>&nbsp; {field.label}
              </div>
